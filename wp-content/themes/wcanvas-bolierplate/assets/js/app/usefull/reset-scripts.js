import cloneAttributes from "./clone-html-attr";
...
class ResetScripts {
    ...
    reloadLazyload() {
        ...
        scripts.setAttribute("id", "wcanvas-boilerplate-lazy-load-js");
        document.body.appendChild(scripts);

        //Remove prev element
        scripts.onload = function () {
            document.querySelector('#wcanvas-boilerplate-lazy-load-js').remove();
        }
    }
    ...
    reloadWPFooter() {
        ...
        [...oldTags].forEach(tag => {
            if (tag.getAttribute('id') != 'wcanvas-boilerplate-main-js') {
                tag.remove();
            }
        });
